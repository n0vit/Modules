...
class MongoStorage(BaseStorage):
    def __init__(
        self, client: "motor_asyncio.AsyncIOMotorClient", db_name, prefix: str, ttl: int
    ):
        self._ttl = ttl
        ...
        if loop.is_closed():
            ...
            threading.Thread(
                ...
                name=f"Message Chain {prefix} init",
                args=(client, db_name, prefix, ttl),
            ).run()
    ...
    async def _create_collection(
        self, db: "motor_asyncio.AsyncIOMotorDatabase", prefix: str, ttl: int
    ):
        try:
            ...
            if "expireAt" not in await self._list_index_names(db, prefix):
                ...
                await db.command(
                    "collMod",
                    prefix,
                    index={"keyPattern": {"expireAt": 1}, "expireAfterSeconds": ttl},
                )

        except OperationFailure:
            pass

    async def _list_index_names(
        self, db: "motor_asyncio.AsyncIOMotorDatabase", prefix: str
    ) -> List[str]:
        names = []
        ...
        async for index in db[prefix].list_indexes():
            index = json.loads(
                json.dumps(
                    index, default=lambda item: getattr(item, "__dict__", str(item))
                )
            )
    ...
    async def get_all_chain(self) -> List[ChainModel]:
        ...
        chain = await cursor.to_list(length=50)
        list = [ChainModel.parse_obj(_) for _ in chain]
        return list
